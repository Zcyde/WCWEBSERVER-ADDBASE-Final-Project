      ...
      <div class="space-y-4">
        <div v-if="plan.plannerFiles && plan.plannerFiles.length" class="mb-4">
          ...
          <ul class="space-y-2 max-h-40 overflow-y-auto pr-2 border p-2 rounded-md bg-gray-50">
            <li v-for="(file, index) in plan.plannerFiles" :key="file.url" 
            ...
            >
              ...
              <button 
                @click="$emit('remove-file', plan.id, index)"
...
<script setup>
import { ref } from "vue";
import { store } from "../eventStore.js"; // Assuming this exists
...
const emit = defineEmits(["close", "update-plan", "remove-file"]); // Added update-plan and remove-file for future
...
const uploadFiles = async () => {
  ...
  // 1. Upload files to a storage service (e.g., AWS S3, Firebase Storage, your own server).
  // 2. Receive an array of public URLs and file names/metadata.
  
  console.log(`Simulating upload for ${uploadingFiles.value.length} files...`);

  // Create mock file objects for the plan
  const newFileMetadata = uploadingFiles.value.map(file => ({
    name: file.name,
    url: `/path/to/uploaded/${props.plan.id}/${file.name}`, // Mock URL
    type: file.type,
    size: file.size,
  }));
  ...
  // 3. Update the planner in the central store
  // We'll merge the new files with existing ones, creating the 'U' (Update) action.
  const updatedPlan = {
      ...props.plan,
      // Ensure 'plannerFiles' exists, then append new files
      plannerFiles: [...(props.plan.plannerFiles || []), ...newFileMetadata]
  };
  
  // Call the store action to update the event/plan
  try {
    // You would need a store action like this in eventStore.js
    await store.updateEvent(updatedPlan); 
