    ...
    >
      ...
      >
          ...
          <div class="flex flex-col space-y-4">
            ...
            <div v-if="selectedPlan?.plannerFiles?.length">
              ...
              <div class="space-y-2 max-h-48 overflow-y-auto pr-2">
                <button
                  v-for="file in selectedPlan.plannerFiles"
                  :key="file.url"
        ...
        <div v-else-if="currentActionView === 'file-update'">
          ...
          <div v-if="selectedPlan?.plannerFiles?.length" class="mb-4 max-h-40 overflow-y-auto border p-3 rounded-lg bg-gray-50">
            <h5 class="text-sm font-semibold text-gray-600 mb-2">Currently Attached:</h5>
            <ul class="space-y-1">
              <li v-for="(file, index) in selectedPlan.plannerFiles" :key="file.url" class="flex items-center justify-between text-sm text-gray-700 bg-white p-2 rounded-md shadow-sm">
                <span class="truncate">{{ file.name }}</span>
                <button 
        ...
        <div v-else-if="currentActionView === 'file-viewer'">
          ...
          <h4 class="text-xl font-bold text-center mb-4 truncate text-blue-600">{{ selectedFile?.name }}</h4>

          <div class="h-[60vh] border border-gray-300 rounded-lg overflow-hidden">
          ...
          <div class="flex justify-center mt-6">
            <button
              @click="switchToTimer"
              class="py-3 px-6 rounded-xl text-base font-bold bg-green-500 text-white shadow hover:bg-green-600 transition flex items-center space-x-2"
            >
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span>Start Timer After Review</span>
            </button>
          </div>
        </div>
      </div>
...
const removePlannerFile = (index) => {
  if (selectedPlan.value?.plannerFiles) {
    // Note: We don't remove it fromsaveUpdatedFiles the actual store yet, just from the selectedPlan's local copy
    selectedPlan.value.plannerFiles.splice(index, 1);
...
const saveUpdatedFiles = async () => {
    ...
    const updateObject = {
        ...
        ...planData, 
        // CRITICAL: Ensure the plannerFiles array is correct (it holds the retained files)
        plannerFiles: planData.plannerFiles || [],
    };
