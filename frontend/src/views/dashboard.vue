<template>
  <div class="flex-1 p-3 sm:p-6 relative bg-gray-50 min-h-screen">
    <!-- Header -->
    <div
      class="relative z-10 p-2 sm:p-4 mb-4 sm:mb-8 bg-gray-300 rounded-full border-2 border-black shadow-md"
    >
      <h1 class="text-lg sm:text-2xl font-bold text-center text-gray-900">
        Dashboard
      </h1>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
      ...
      >
        ...
        <div class="text-center sm:text-left">
          <h2 class="text-sm sm:text-xl text-gray-500 font-medium">
            Welcome Back,
          </h2>
          <h1 class="text-base sm:text-3xl font-bold text-gray-800">
            {{ firstName }} {{ lastName }}
          </h1>
        </div>
      </div>
      ...
      <div v-if="activeTasks.length" class="space-y-2 sm:space-y-3">
        ...
        >
          ...
          <div class="flex-shrink-0 flex items-center space-x-1 sm:space-x-3">
            <span class="text-xs sm:text-sm font-medium text-gray-500 bg-gray-200 px-2 py-0.5 rounded-full">
              {{ plan.date }}
            </span>
            <span
              v-if="plan.time"
    ...
    <div class="bg-white p-3 sm:p-4 rounded-lg shadow-xl">
      ...
      <!-- Mobile: stacked vertically | Desktop: 7 columns -->
