<template>
  <div class="relative p-4 md:p-8 min-h-screen bg-gray-100">
    <!-- Header -->
    <div class="flex flex-col md:flex-row justify-between items-center border-b pb-4 mb-6 space-y-4 md:space-y-0 relative z-10">
      <h1
    ...
    <div v-if="uploadedFiles.length > 0" class="flex flex-wrap gap-6 relative z-10">
      ...
      >
        <div class="w-20 h-16 md:w-28 md:h-24 bg-blue-400 rounded-lg shadow-md flex justify-center items-center text-2xl">
          📄
        </div>
        <span class="mt-2 text-sm md:text-base text-center truncate max-w-[6rem]">
          {{ file.name }}
        </span>
        <a
    ...
    <!-- Watermark -->
    <div
      class="absolute inset-0 flex justify-center items-center pointer-events-none opacity-10 z-0"
    >
      <img src="../test.png" alt="watermark" class="w-[20rem] md:w-[30rem]" />
    </div>
  </div>
</template>
